...
OS_QtWindows::OS_QtWindows(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::OS_QtWindows)
{
    ui->setupUi(this);
     ...
     * 总体窗口部分
     */
    // tip WSL中文乱码的问题属于WSL的锅，因此本操作系统不考虑支持中文
    QString title = QString::fromLocal8Bit("OS:Windows-qt6");
     ...
     * 状态栏按钮点击事件
     */
    StatusBarLeftClicked();
    StatusBarRightClicked();
...
OS_QtWindows::~OS_QtWindows()
{
    delete ui;
}

void OS_QtWindows::close()
{
}

void OS_QtWindows::notify(QEvent *event)
{
...
void OS_QtWindows::resizeEvent(QResizeEvent *event)
{
    ...
    if (statusBar)
    { // 确保 statusBar 已经被初始化
        statusBar->resize(this->width(), 50);
...
void OS_QtWindows::showWinWindow()
{
    // 传入win_btn的位置
